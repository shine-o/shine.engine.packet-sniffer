package main

import (
	"bytes"
	"encoding/binary"
	"fmt"
	"github.com/google/gopacket"
	"github.com/google/gopacket/layers"
	"github.com/google/gopacket/pcapgo"
	"os"
)

var xorKey [499]byte

/* 8990
	union Name256Byte
	{
	char n256_name[256];
	unsigned __int64 n256_code[32];
	};
*/
type Name256Byte struct {
	n256_name [256]string
	n256_code uint64
}

/* 3801
	union Name4
	{
	char n4_name[16];
	unsigned int n4_code[4];
	};
*/
type Name4 struct
{
	n4_name [16]string
	n4_code uint
};

/*
	struct PROTO_NC_USER_LOGIN_REQ
	{
	Name256Byte user;
	Name4 password;
	};
 */
type ProtoNcUserLoginReq struct
{
	user Name256Byte
	password Name4
}

func init()  {
	xorKey = [499]byte{
		,
	}
}

func main() {
	f, err := os.Open("Client-Login.pcapng")

	if err != nil {
		fmt.Println(err)
	}

	defer f.Close()

	r, err := pcapgo.NewNgReader(f, pcapgo.DefaultNgReaderOptions)
	if err != nil {
		fmt.Println(err)
	}
	//b := hex.DecodeString("0492268a328b0492268a328b08004500006bf62b400080067f60c0a801b8c0a801f812042332f095520bbaf8c3d250187ffb5d7300004242c0c80e109365f9ef31685d7ba223a385fd4c05abde6032a7c55aa7036a065646a9880bf821c634f09ef6bfbdcf954e63001fbc3bfc11f90c2e12fba774fa89bb40")
	for p := range gopacket.NewPacketSource(r, layers.LayerTypeEthernet).Packets() {
		//fmt.Println(p)
		if p.ApplicationLayer() == nil {
			continue
		}

		pp := p.ApplicationLayer().LayerContents()

		dpp := xorCipher(pp,len(pp))

		br := bytes.NewReader(dpp)

		//br.ReadAt(pp, 3)
		/*
			uint16 command = (department << 10) | command & 1023
		 */

		var opCode uint16
		err = binary.Read(br, binary.LittleEndian, &opCode)

		//fmt.Println(opCode)
		header := byte(opCode >> 10)
		ptype := byte(opCode & 1023)

		fmt.Printf("OpCode: %x, \n Header: %d, \n Type: %d \n", opCode, header, ptype)

		//opCode = uint16((header << 10) | ptype & 1023)
		if err != nil {
			fmt.Println(err)
		}
	}

	fmt.Println("I have read all packets")
}

func xorCipher(b []byte, len int) []byte {
	xorLen := 0
	nb := make([]byte, len)
	for i := 0; i < len; i++ {
		nb[i] = b[i] ^ xorKey[xorLen]
		if xorLen == 14 {
			xorLen = 0
		}
	}
	return nb
}